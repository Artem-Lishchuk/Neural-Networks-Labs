...
class TwoLayerNet(object):
    ...
    self.params that maps parameter names to torch tensors.
    """
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10, weight_scale=1e-3, reg=0.0):
        ...
        self.params['W1'] = weight_scale * torch.randn(input_dim, hidden_dim, dtype=torch.float64)
        self.params['b1'] = torch.zeros(hidden_dim, dtype=torch.float64)
        self.params['W2'] = weight_scale * torch.randn(hidden_dim, num_classes, dtype=torch.float64)
        self.params['b2'] = torch.zeros(num_classes, dtype=torch.float64)
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        h, cache = affine_relu_forward(X, self.params['W1'], self.params['b1'])
       ...
       # If y is None then we are in test mode so just return scores
        ...
        dX, grads['W1'], grads['b1'] = affine_relu_backward(dh, cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10, dropout=0, use_batchnorm=False, reg=0.0, weight_scale=1e-2, dtype=torch.float64, seed=None):
        ...
        - weight_scale: Scalar giving the standard deviation for random
          initialization of the weights.
        - dtype: A torch datatype object; all computations will be performed using
          this datatype.
        ...
        for i in range(self.num_layers - 1):
            ...
            this_dim = hidden_dims[i]

            self.params['W%d' % (i + 1)] = weight_scale * torch.randn(prev_dim, this_dim, dtype=self.dtype)
        ...
        self.params['W%d' % self.num_layers] = weight_scale * torch.randn(prev_dim, num_classes, dtype=self.dtype)
        self.params['b%d' % self.num_layers] = torch.zeros(num_classes, dtype=self.dtype)
    ...
    def loss(self, X, y=None):
        ...
        scores, cache = affine_forward(h, self.params['W%d' % self.num_layers], self.params['b%d' % self.num_layers])
        caches.append(cache)
        ...
        dh, grads['W%d' % (self.num_layers)], grads['b%d' % (self.num_layers)] = affine_backward(dscores, cache)
        ...
        for i in reversed(range(self.num_layers - 1)):
            ...
            if self.use_batchnorm:
                ...
                cache = caches.pop()
                dh, grads['W%d' % (i + 1)], grads['b%d' % (i + 1)] = affine_relu_backward(dh, cache)
